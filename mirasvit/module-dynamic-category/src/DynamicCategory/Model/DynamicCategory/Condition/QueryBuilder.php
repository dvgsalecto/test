...
class QueryBuilder
{
    ...
    public function joinField(Select $select, string $fieldName): string
    {
        ...
        if (!$this->isJoined($select, $field)) {
            ...
            if ($this->registry->getCategoryStoreIds()) {
                $condition .= " AND {$tableAlias}.store_id IN (" . implode(',', $this->registry->getCategoryStoreIds()) . ")";
            }
    ...
    public function isJoined(Select $select, string $field): bool
    {
        return strpos((string)str_replace('`', '', $select->__toString()), $field) !== false;
    }
}
